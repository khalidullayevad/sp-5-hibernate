...
public class AmountServiceImpl implements AmountService {
    ...
    public List<Amount> sortAmountWithBdate(String sortBy, String sortOrder, int page, Date at, Date from) {
        if("count".equals(sortBy)){
            ...
            return entityManager.createQuery("Select new com.example.sp5.entites.Amount(a, count(a)) FROM  Author  a, Book b where a.birthdate BETWEEN  "
                            +at+ " and "+
                            from+" AND a.id = b.author.id GROUP BY a.name ORDER by a."+sortBy+" "
                            +sortOrder)
                    .setFirstResult(page * 2)
                    .setMaxResults(page*2+2)
                    .getResultList();
